...
(function () {
    ...
    function setData(d){
        ...
        //user.money+=a.money
        //user.xp+=a.xp
        //user.tokens=a.tokens
        //user.tickets=a.tickets
	...
	function initMenu() {
        ...
        options.giveAllItems.btn.onclick = () => {
            ic=0
            liit = 0;
            nf("Fetching all item lists. This may take a few seconds. (Page will refresh after It's finished)")
            itemNames.forEach(name => {
                ...
                fetch(`https://csgo.mtsl.dk/data/items/${name}.json?v=15`).then(res => res.json()).then(data => {
                    ...
                    log(`Fetching item list '${name}'.`, "yellow")
                    if(liit == itemNames.length){
                        log(`Successfully fetched all item lists.`, "green")
                        nf("Successfully fetched all item lists.")
                        save(true)
                    }
                })
            })

            log(`Fetching ${ic} item lists.`, "yellow")
		};
        ...
        options.setPassiveIncome.btn.onclick = () => {
            ...
            const valid = Number(value)!=NaN ? true : false

            if(!valid){
                log("Unable to parse number for passiveIncome","red")
                nf("This input is invalid.",null,"red")
                return
            }
    ...
    //SETUP
    try {
        //GM_setValue("moPsEk.notif1",false)
        /*if(!GM_getValue("moPsEk.notif1") == true){
            GM_setValue("moPsEk.notif1",true)
            let c = confirm("Due a current bug, your current inventory might be corrupted.\n\nPlease confirm that you want to play with moPsEk!\n(Press cancel to play without moPsEk)")
            if(c==false) return log("Cancelled moPsEk initialization.","red");
        }*/
